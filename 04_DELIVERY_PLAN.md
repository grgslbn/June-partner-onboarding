...
- Production domain setup (TBD — staging runs on `pos.june-energy.app`; production cutover to `onboard.june.energy` pending June confirmation).
...
- [ ] DNS for production domain pointing to Vercel, SSL cert issued (staging: `pos.june-energy.app`; production: TBD, likely `onboard.june.energy`).
