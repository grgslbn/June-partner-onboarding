...
export async function runDailyDigest(): Promise<void> {
  ...
  const normalised: LeadRow[] = leads.map((l) => {
    ...
    return {
      ...
      address_street:       (l.address as { street?: string } | null)?.street ?? null,
      address_postal_code:  (l.address as { postal_code?: string } | null)?.postal_code ?? null,
      address_city:         (l.address as { city?: string } | null)?.city ?? null,
      is_business:          l.is_business != null ? String(l.is_business) : null,
      business_name:        l.business_name ?? null,
      business_vat:         l.business_vat ?? null,
      sepa_accepted:        l.sepa_accepted != null ? String(l.sepa_accepted) : null,
      housing_type:         l.housing_type ?? null,
      birth_date:           l.birth_date ?? null,
      billing_frequency:    l.billing_frequency ?? null,
      product_choice:       l.product_choice ?? null,
    };
  });
